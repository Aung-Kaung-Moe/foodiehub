...
export default function Profile({ showToast, user, onUserUpdate }) {
  ...
  // also push updated user up so navbar profile icon changes
  const saveAvatar = async () => {
    if (!preview?.file) return;
    try {
      await apiProfileAvatar(preview.file);
      // refresh user to get new avatar_url
      const { user: updatedUser } = await apiProfile();
      setMe(updatedUser);
      // 🔥 update global user in App so Navbar/ProfileMenu re-render with new avatar
      if (onUserUpdate) {
    ...
    <section className="py-10">
      <div className="mx-auto max-w-3xl space-y-8">
        ...
        <form
          ...
          className="rounded-3xl bg-neutral-900 p-6 ring-1 ring-neutral-800 space-y-4"
        >
